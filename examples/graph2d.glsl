...
vec4 getVoxel(vec3 pos) {
  ...
  // tangent line
  if (abs(tlan(mod(time / 4.0, 16.0), pos.x / scale.x) * scale.y - pos.y) < 1.0 && pos.z >= 20.0) return vec4(0.0, 1.0, lum(pos), 1.0);
  // f(x)
  ...
  if (abs(pos.x - mod(time / 4.0, 16.0) * scale.x) < 1.0 && abs(pos.y - f(mod(time / 4.0, 16.0)) * scale.y) < 1.0 && pos.z >= 8.0) return vec4(lum(pos), 0.0, 1.0, 1.0); // point indicator
